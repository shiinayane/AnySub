...
![AnySub 在视频上渲染注音与语义排版](./docs/hero.gif)

> 也是一个称职的**通用**字幕挂载器:把任意 SRT / VTT / ASS / SSA 拖到任意视频上、调好样式即可。日语相关功能都是选开的,不打扰。

## 为什么用它学日语

- 🈁 **逐字精准注音。** 文本字幕里 `温厚（おんこう）`/`使徒《しと》` 显示为汉字上方假名。与粗糙工具不同,AnySub **把读音对齐到正确的汉字**:`近接猟兵（りょうへい）` 只把 りょう/へい 注到 `猟兵`、`近接` 留白,而不是铺满整串。内置精简 KANJIDIC2 读音表(常用+人名用,~3000 字)+ 连浊/促音感知的对齐算法——**无数 MB 分词器、零运行时网络**。
- 🎭 **语义排版**(动画字幕向,可开关)。日文 CC 的标点承载语义,AnySub 据此重排(只排版、绝不删字):
  - **话者名** `（マオマオ)台词` → 名字淡化缩小,视线直达台词。
  - **非语音** 独立 `（ドアが開く音)`/`（ざわざわ)`(音效·动作)→ 斜体淡化。
  - **画外音/心声** `〈…〉`/`＜…＞`(电话·旁白·内心)→ 斜体。
  - **书面/引用** `《…》`(书信·画面读字;与注音 `漢字《かな》` 消歧)→ 切衬线体,像"写出来的字"。
  - **歌词** 行首 `♪` → 斜体。
  - **跨行/跨 cue 跨度**:`〈…` 开在一句、`…〉` 闭在几句之后,中间无括号的行也连续标记(`《》`、`♪…♪` 同理)。
- 🔍 **一键在线字幕([Jimaku](https://jimaku.cc))。** `Alt+Shift+F` → 搜番剧 → 选番剧 → 选文件 → 一键挂载。半自动:始终列出候选,绝不静默加载错字幕;番剧名经 AniList 定位,ASS 优先。搜索框从页面标题**智能预填**番剧名+集数。
- ⏭️ **切集自动接续。** SPA 换集(页面标题集数变化)→ 自动清除旧字幕 →**同源优先**自动加载下一集(跨集匹配同一字幕组/压制源);找不到同源才弹候选。追番零手工。

## 其余功能

- 📂 本地字幕文件(选择 / 拖拽 / **清除**),**文件不出本机**(SRT/VTT 全程离线)。
- 🎬 支持 **SRT / VTT / ASS / SSA**。
- ✨ **ASS/SSA 高保真渲染**:懒加载 [libass-wasm](https://github.com/libass/JavascriptSubtitlesOctopus),还原斜体/粗体/描边/定位/特效/字体;**加载失败(无网/CSP)自动降级为纯文本**,字幕始终可见。
- 🎨 **自绘覆盖层渲染**:样式完全可控且跨浏览器一致(不受 Safari `::cue` 限制)。
  - 背景:描边 / **半透明(默认)** / 黑底 / 无;颜色:白 / 黄 / 青 / 绿。
  - **字号随播放器高度等比缩放**,窗口/全屏观感一致;边距可调。
- 🧭 **说话在下,非语音在上**(心智统一):「说话位置 底/顶」切换台词锚点(默认底部);说话(对白/话者名/画外音/歌词)放该锚点、多说话人叠放靠话者名区分;仅**真·音效**(独立 `（…)`)恒放对侧。
...
- 🪶 **零空闲开销**:未加载字幕且未开悬浮球时不连接任何观察器/定时器,注入每页几乎无成本。
- 💾 **设置持久化**:字号 / 位置 / 背景 / 颜色 / 语言偏好自动记住(localStorage,按站点)。
...
1. 打开任意带视频的网页。
2. 按 **`Alt+Shift+S`**(或 `Ctrl+Shift+S`)打开面板——或在面板里开启悬浮球。
3. 「选择文件」/ 把字幕拖到面板——或「在线字幕」(`Alt+Shift+F`)从 Jimaku 获取。
4. 按需调偏移 / 字号。开启 **日文注音** 与 **话者·音效标记** 使用日语相关功能。
...
**注音逐字对齐**不走通用分词器(kuromoji 那类需运行时加载数 MB 词典),而是内置一张从 [KANJIDIC2](https://www.edrdg.org/kanjidic/kanjidic2.xml.gz) 抽取的精简「汉字→读音」表(常用+人名用约 3000 字,平假名归一,gzip ~35KB),随脚本本地存储、首次注音才 `JSON.parse`。对齐用带记忆化的递归匹配:拿每个汉字的读音候选去消费括号里的假名串,动态生成连浊(は→ば)、促音(がく→がっ)变体;整串对不齐时从左逐字剥离取最长覆盖解,再对不齐则回退整串注音(熟字訓如 `今日→きょう`)。
...
- [x] ~~逐字注音对齐(KANJIDIC2)~~(v0.13.0)
- [x] ~~语义排版 + 说话/非语音定位~~(v0.14.0)
- [x] ~~界面 i18n(English / 中文 / 日本語)~~(v0.15.0)
- [ ] 跨域 iframe 内视频支持
