...
test('整串音读对齐 温厚→おんこう', () => {
  ...
  assert.deepEqual(r.pairs, [['温', 'おん'], ['厚', 'こう']]);
});

test('单字 使徒→しと', () => {
  const r = alignFurigana('使徒', 'しと');
  assert.equal(r.plain, '');
  assert.deepEqual(r.pairs, [['使', 'し'], ['徒', 'と']]);
});
...
test('后缀读音 近接猟兵→りょうへい 只注 猟兵', () => {
  ...
  assert.deepEqual(r.pairs, [['猟', 'りょう'], ['兵', 'へい']]);
});
...
test('连浊 立場→たちば(場: は→ば)', () => {
  ...
  assert.equal(r.plain, '');
  assert.deepEqual(r.pairs, [['立', 'たち'], ['場', 'ば']]);
});

test('连浊 花火→はなび(火: ひ→び)', () => {
  const r = alignFurigana('花火', 'はなび');
  assert.equal(r.plain, '');
  assert.deepEqual(r.pairs, [['花', 'はな'], ['火', 'び']]);
});
...
test('促音 学校→がっこう(学: がく→がっ)', () => {
  ...
  assert.equal(r.plain, '');
  assert.deepEqual(r.pairs, [['学', 'がっ'], ['校', 'こう']]);
});
