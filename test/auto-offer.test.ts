...
test('占位元素 dur:NaN → 排除', () => {
  assert.equal(
    isFeatureVideo(
      { duration: NaN, paused: true, currentTime: 0, muted: false, volume: 1, ...rect(0, 0) },
      VW,
      VH,
    ),
    false,
...
test('静音自动播放的 hero 预览(小尺寸)→ 排除', () => {
  assert.equal(
    isFeatureVideo(
      { duration: 600, paused: false, currentTime: 5, muted: true, volume: 1, ...rect(600, 200) },
      VW,
      VH,
    ),
    false,
...
test('静音但占大半视口(主播放器,静音观看)→ 命中(兜底)', () => {
  assert.equal(
    isFeatureVideo(
      { duration: 600, paused: false, currentTime: 5, muted: true, volume: 1, ...rect(1280, 720) },
      VW,
      VH,
    ),
    true,
...
test('短视频(<120s)即使在播也不算正片', () => {
  assert.equal(
    isFeatureVideo(
      { duration: 45, paused: false, currentTime: 5, muted: false, volume: 1, ...rect(1280, 720) },
      VW,
      VH,
    ),
    false,
